...
COMMAND *parseCommandLine(char *cmdLine) 
{
	...
	char delimiter[] = " \t";
	argvPos[argc] = strtok(cmdLine, delimiter); // zeigt auf dan token, das mit \0 endet
	do
	{
		argc++;
		argvPos[argc] = strtok(NULL, delimiter); // zeigt auf dan token, das mit \0 endet
	}while (argvPos[argc] != NULL && argc < 32);
	/** END TODO */
